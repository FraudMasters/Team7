"""
Indeed XML Import Service

This module provides Indeed XML parsing and import capabilities for candidate data.

Features:
- Parse Indeed XML export files
- Extract candidate application information (name, email, phone, resume, etc.)
- Validate XML format and required fields
- Handle multiple Indeed XML export formats
- Comprehensive error reporting and validation

The service supports standard Indeed XML exports including:
- Application exports (candidate applications)
- Resume exports (candidate profiles)
- Job posting applicant lists
"""
import logging
...
@dataclass
class IndeedApplicationData:
    """
    Parsed Indeed application data from XML.
    ...
    Attributes:
        ...
        indeed_profile_url: Link to Indeed profile
...
class XMLParseResult:
    ...
    Attributes:
        ...
        detected_format: Detected Indeed XML format type
    """
    ...
    applications: List[IndeedApplicationData] = field(default_factory=list)
    errors: List[str] = field(default_factory=list)
    warnings: List[str] = field(default_factory=list)
    detected_format: Optional[str] = None


class IndeedXMLImporter:
    """
    Service for parsing Indeed XML exports and extracting candidate data.

    This service handles various Indeed XML export formats and normalizes
    the data into a consistent structure for import into the ATS.

    Indeed XML formats supported:
    - Application export (applicants for job postings)
    - Resume export (candidate profiles with resume data)
    - Bulk applicant export (multiple applications)
    """

    # XML element name mappings across different Indeed XML formats
    ELEMENT_MAPPINGS = {
        # Applicant ID variants
        "applicant_id": ["ApplicantId", "applicant_id", "id", "Id", "ApplicationId"],
        # First Name variants
        "first_name": ["FirstName", "first_name", "firstName", "givenName", "GivenName"],
        # Last Name variants
        "last_name": ["LastName", "last_name", "lastName", "surname", "Surname", "familyName"],
        # Email variants
        "email": ["Email", "email", "EmailAddress", "email_address", "contactEmail"],
        # Phone variants
        "phone": ["Phone", "phone", "PhoneNumber", "phone_number", "contactPhone", "mobile"],
        # Resume URL variants
        "resume_url": ["ResumeUrl", "resume_url", "resumeURL", "resumeLink", "ResumeLink"],
        # Resume Text variants
        "resume_text": ["ResumeText", "resume_text", "resumeContent", "ResumeContent", "resume"],
        # Job Title variants
        "job_title": ["JobTitle", "job_title", "jobTitle", "title", "Title", "positionTitle"],
        ...
        # Status variants
        "status": ["Status", "status", "ApplicationStatus", "application_status", "applicationStatus"],
        # Location variants
        "location": ["Location", "location", "city", "City", "address", "Address"],
    ...
    def __init__(self):
        """Initialize the Indeed XML importer service."""
        logger.info("Initialized IndeedXMLImporter")

    def parse_xml_file(self, file_path: str, encoding: str = "utf-8") -> XMLParseResult:
        """
        Parse an Indeed XML file from disk.
        ...
        """
        try:
            logger.info(f"Parsing Indeed XML file: {file_path}")
    ...
    def parse_xml_content(self, content: str) -> XMLParseResult:
        """
        Parse Indeed XML content from a string.
        ...
        """
        try:
            logger.info("Parsing Indeed XML content")
            ...
            # Find application/applicant elements
    ...
    def _detect_xml_format(self, root: ET.Element) -> str:
        """
        Detect the type of Indeed XML export based on root element and structure.
        ...
        if "applications" in root_tag or "applicants" in root_tag:
            return "bulk_applications_export"
        elif "application" in root_tag or "applicant" in root_tag:
            return "single_application_export"
        elif "resumes" in root_tag:
            return "resume_export"
        elif "indeed" in root_tag or "export" in root_tag:
            # Check for child elements
            children = list(root)
            if children:
                first_child_tag = children[0].tag.lower()
                if "application" in first_child_tag or "applicant" in first_child_tag:
                    return "indeed_application_export"
                elif "resume" in first_child_tag:
                    return "indeed_resume_export"

        return "unknown_format"

    def _find_application_elements(self, root: ET.Element) -> List[ET.Element]:
        """
        Find all application/applicant elements in the XML tree.
        ...
        """
        # Common element names for applications/applicants
        element_names = [
            "Application",
            "application",
            "Applicant",
        ...
        # Check if root itself is an application element
        if any(name in root.tag for name in element_names):
            elements.append(root)
            return elements

        # Search for application elements
        for name in element_names:
            ...
            # Try with namespace (common in Indeed exports)
    ...
    def _parse_application_element(self, element: ET.Element) -> IndeedApplicationData:
        """
        Parse a single XML application element into IndeedApplicationData.

        Args:
            element: XML element containing application data

        Returns:
            IndeedApplicationData object with parsed data
        """
        application = IndeedApplicationData()
        raw_data = {}
        ...
        for standard_field, xml_tags in self.ELEMENT_MAPPINGS.items():
            ...
            # Set field value
            if standard_field == "applicant_id":
                application.applicant_id = value
            ...
            elif standard_field == "indeed_profile_url":
                application.indeed_profile_url = value
        ...
        if not application.first_name and not application.last_name:
            name_fields = ["Name", "name", "CandidateName", "candidate_name", "FullName", "full_name"]
    ...
    def _parse_full_name(self, application: IndeedApplicationData, full_name: str):
        """
        Parse a full name into first_name and last_name.

        Args:
            application: IndeedApplicationData to update
    ...
    def _validate_application(self, application: IndeedApplicationData) -> Dict[str, Any]:
        """
        Validate that an application has required fields.

        Args:
            application: IndeedApplicationData to validate
    ...
    def get_supported_formats(self) -> List[str]:
        """
        Get list of supported Indeed XML formats.

        Returns:
            List of supported format names
        """
        return [
            "bulk_applications_export",
            "single_application_export",
            "resume_export",
            "indeed_application_export",
            "indeed_resume_export",
        ]
    ...
    def validate_xml_structure(self, root: ET.Element) -> Dict[str, Any]:
        ...
        if not application_elements:
            return {
                "valid": False,
                "error": "No application/applicant elements found in XML",
                "detected_format": self._detect_xml_format(root),
            }
        ...
        if application_elements:
            ...
            if not validation_result["valid"]:
                return {
                    "valid": False,
                    "error": f"First application failed validation: {validation_result['error']}",
                    "detected_format": self._detect_xml_format(root),
                }
