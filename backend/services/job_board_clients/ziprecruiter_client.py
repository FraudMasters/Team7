"""
ZipRecruiter API Client for fetching applicant resumes

This module provides a client for interacting with the ZipRecruiter API to fetch
...
- Support for ZipRecruiter API authentication
...
@dataclass
class ZipRecruiterApplicant:
    """
    ZipRecruiter applicant data structure.

    Attributes:
        applicant_id: Unique identifier for the applicant in ZipRecruiter
        ...
        metadata: Additional metadata from ZipRecruiter
...
@dataclass
class ZipRecruiterFetchResult:
    """
    Result of fetching applicants from ZipRecruiter.
    ...
    applicants: List[ZipRecruiterApplicant]
    total_count: int
    page: int
    page_size: int
    has_more: bool
...
class ZipRecruiterClient:
    ...
    This client provides methods to fetch applicant resumes from ZipRecruiter
    for specified job postings, handling authentication, pagination,
    and error recovery.

    The ZipRecruiter API uses API key authentication and provides endpoints
    for retrieving applicants for job postings managed through the
    ZipRecruiter employer platform.
    """

    # ZipRecruiter API endpoints
    BASE_URL = "https://api.ziprecruiter.com/v1"
    ...
    def __init__(
        ...
        """
        Initialize the ZipRecruiter API client.

        Args:
            api_key: ZipRecruiter API key for authentication
        ...
        logger.info(f"ZipRecruiterClient initialized with base_url: {self.base_url}")
    ...
    async def fetch_applicants(
    ...
    ) -> ZipRecruiterFetchResult:
        """
        Fetch applicants for a specific job posting from ZipRecruiter.

        Args:
            job_id: ZipRecruiter job ID to fetch applicants for
        ...
        Returns:
            ZipRecruiterFetchResult with applicants and metadata
        ...
        try:
            ...
            # Validate API key
            if not self.api_key:
                raise ValueError("ZipRecruiter API key is not configured")
            ...
            # Convert to ZipRecruiterApplicant objects
            applicants = []
            ...
            return ZipRecruiterFetchResult(
    ...
    async def fetch_all_applicants(
        ...
        Returns:
            ZipRecruiterFetchResult with all applicants and metadata
        """
        try:
    ...
    async def download_resume(self, resume_url: str) -> bytes:
        """
        Download a resume file from ZipRecruiter.
    ...
    def _parse_applicant_data(self, data: Dict[str, Any]) -> ZipRecruiterApplicant:
        """
        Parse applicant data from ZipRecruiter API response.

        Args:
            data: Raw applicant data from ZipRecruiter API

        Returns:
            ZipRecruiterApplicant object
    ...
    async def close(self):
        ...
        logger.info("ZipRecruiterClient closed")
...
async def get_ziprecruiter_client_from_integration(
    integration: JobBoardIntegration,
) -> ZipRecruiterClient:
    """
    Create a ZipRecruiterClient from a JobBoardIntegration model.

    Args:
        integration: JobBoardIntegration with ZipRecruiter credentials

    Returns:
        Configured ZipRecruiterClient instance
    ...
    return ZipRecruiterClient(
        api_key=integration.api_key,
        api_endpoint=integration.api_endpoint or None,
    )


def get_ziprecruiter_client() -> Optional[ZipRecruiterClient]:
    """
    Create a ZipRecruiterClient using global settings.
    ...
    Returns:
        ZipRecruiterClient instance if API key is configured, None otherwise

    Example:
        >>> from services.job_board_clients.ziprecruiter_client import get_ziprecruiter_client
        >>>
        >>> client = get_ziprecruiter_client()
        >>> if client:
        >>>     result = await client.fetch_applicants(job_id="12345")
    """
    settings = get_settings()

    # Note: ZipRecruiter API key should be added to settings if needed globally
    # For now, we use the JobBoardIntegration model for per-organization config
    api_key = getattr(settings, 'ziprecruiter_api_key', None)

    if not api_key:
        logger.warning("ZipRecruiter API key not configured in settings")
        return None

    return ZipRecruiterClient(api_key=api_key)
