"""
Glassdoor API Client for fetching applicant resumes

This module provides a client for interacting with the Glassdoor API to fetch
...
- Support for Glassdoor API authentication
...
@dataclass
class GlassdoorApplicant:
    """
    Glassdoor applicant data structure.

    Attributes:
        applicant_id: Unique identifier for the applicant in Glassdoor
        ...
        metadata: Additional metadata from Glassdoor
...
@dataclass
class GlassdoorFetchResult:
    """
    Result of fetching applicants from Glassdoor.
    ...
    applicants: List[GlassdoorApplicant]
    total_count: int
    page: int
    page_size: int
    has_more: bool
...
class GlassdoorClient:
    ...
    This client provides methods to fetch applicant resumes from Glassdoor
    for specified job postings, handling authentication, pagination,
    and error recovery.

    The Glassdoor API uses API key authentication and provides endpoints
    for retrieving applicants for job postings managed through the
    Glassdoor employer platform.
    """
    ...
    def __init__(
        ...
        """
        Initialize the Glassdoor API client.

        Args:
            api_key: Glassdoor API key for authentication
        ...
        logger.info(f"GlassdoorClient initialized with base_url: {self.base_url}")
    ...
    async def fetch_applicants(
    ...
    ) -> GlassdoorFetchResult:
        """
        Fetch applicants for a specific job posting from Glassdoor.

        Args:
            job_id: Glassdoor job ID to fetch applicants for
        ...
        Returns:
            GlassdoorFetchResult with applicants and metadata
        ...
        try:
            ...
            # Convert to GlassdoorApplicant objects
            applicants = []
    ...
    async def fetch_all_applicants(
        ...
        Returns:
            GlassdoorFetchResult with all applicants and metadata
        """
        try:
    ...
    async def download_resume(self, resume_url: str) -> bytes:
        """
        Download a resume file from Glassdoor.
    ...
    def _parse_applicant_data(self, data: Dict[str, Any]) -> GlassdoorApplicant:
        """
        Parse applicant data from Glassdoor API response.

        Args:
            data: Raw applicant data from Glassdoor API

        Returns:
            GlassdoorApplicant object
    ...
    async def close(self):
        ...
        logger.info("GlassdoorClient closed")
...
async def get_glassdoor_client_from_integration(
    integration: JobBoardIntegration,
) -> GlassdoorClient:
    """
    Create a GlassdoorClient from a JobBoardIntegration model.

    Args:
        integration: JobBoardIntegration with Glassdoor credentials

    Returns:
        Configured GlassdoorClient instance
...
def get_glassdoor_client() -> Optional[GlassdoorClient]:
    """
    Create a GlassdoorClient using global settings.
    ...
    Returns:
        GlassdoorClient instance if API key is configured, None otherwise

    Example:
        >>> from services.job_board_clients.glassdoor_client import get_glassdoor_client
        >>>
    ...
    # Note: Glassdoor API key should be added to settings if needed globally
    # For now, we use the JobBoardIntegration model for per-organization config
    api_key = getattr(settings, 'glassdoor_api_key', None)

    if not api_key:
        logger.warning("Glassdoor API key not configured in settings")
        return None

    return GlassdoorClient(api_key=api_key)
