"""
Database connection management for Taxonomy Service.

This module provides the database engine, session factory, and dependency injection
for FastAPI endpoints. Also includes SQLAlchemy event listeners for automatic
...
def _extract_table_and_operation(query: str) -> tuple[str, str]:
    """
    Extract SQL operation type and table name from query.

    Args:
        query: SQL query string

    Returns:
        Tuple (operation, table_name)
        - operation: SELECT, INSERT, UPDATE, DELETE or OTHER
        - table_name: Extracted table name or 'unknown'

    Example:
        >>> _extract_table_and_operation("SELECT * FROM skill_taxonomies WHERE id = 1")
        ('SELECT', 'skill_taxonomies')
    """
    try:
...
def _before_cursor_execute(conn, cursor, statement, parameters, context, executemany):
    """
    Event listener before cursor execution.

    Records start time for each query for performance tracking.
...
def _after_cursor_execute(conn, cursor, statement, parameters, context, executemany):
    """
    Event listener after cursor execution.

    Calculates query execution time and logs performance metrics.
    Called after each database query completes.
...
async def get_db() -> AsyncGenerator[AsyncSession, None]:
    ...
    sessions to endpoints. Automatically handles session cleanup.
