"""
Конфигурация приложения для Analytics Service.
...
class Settings(BaseSettings):
    ...
    Attributes:
        ...
        reports_path: Путь для сохранения сгенерированных отчетов
    """
    ...
    database_url: str = Field(
        default="postgresql://postgres:postgres@localhost:5432/analytics_service",
    ...
    # Reports Configuration / Конфигурация отчетов
    reports_path: Path = Field(
        default=Path("./reports"),
        description="Путь для сохранения сгенерированных отчетов",
    )
...
def get_settings() -> Settings:
    ...
    if _settings is None:
        _settings = Settings()
        logger.info(f"Loaded Analytics Service configuration (log_level={_settings.log_level})")
    return _settings
