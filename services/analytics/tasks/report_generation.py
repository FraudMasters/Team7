"""
Задачи генерации отчетов для плановой аналитики.

# Русский комментарий:
Этот модуль предоставляет задачи Celery для генерации плановых отчетов,
их форматирования для доставки (PDF, CSV и т.д.) и отправки по электронной
почте или другим каналам доставки.
"""
import logging
import time
...
---
This is an automated report from AgentHR Analytics Service.
            """.strip(),
            "attachments": attachments or [],
        }

        logger.info(
            f"Email prepared: subject='{email_details['subject']}', "
...
def generate_scheduled_reports(
    ...
    try:
        ...
        self.update_state(state="PROGRESS", meta=progress)
        logger.info(
            f"Task {self.request.id}: Step {current_step}/{total_steps} - Generating data"
        )
        ...
        self.update_state(state="PROGRESS", meta=progress)
        logger.info(
            f"Task {self.request.id}: Step {current_step}/{total_steps} - Formatting report"
        )
        ...
        self.update_state(state="PROGRESS", meta=progress)
        logger.info(
            f"Task {self.request.id}: Step {current_step}/{total_steps} - Delivering report"
        )
