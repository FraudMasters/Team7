"""
Приложение FastAPI для Analytics Service.
...
async def lifespan(app: FastAPI) -> AsyncGenerator:
    ...
    logger.info(f"Путь для отчетов: {settings.reports_path}")

    # Инициализация директории для отчетов
    settings.reports_path.mkdir(parents=True, exist_ok=True)
    logger.info(f"Директория для отчетов создана: {settings.reports_path}")
...
app = FastAPI(
    ...
    description="Микросервис аналитики и отчетности",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
...
async def readiness_check() -> JSONResponse:
    ...
    # TODO: Добавить проверку доступности директории для отчетов
