"""
Приложение FastAPI для Notification Service.
...
async def lifespan(app: FastAPI) -> AsyncGenerator:
    ...
    logger.info(f"SMTP хост: {settings.smtp_host}")
    logger.info(f"SMS провайдер: {settings.sms_provider}")

    # Инициализация директории для email шаблонов
    settings.email_templates_path.mkdir(parents=True, exist_ok=True)
    logger.info(f"Директория для email шаблонов создана: {settings.email_templates_path}")
...
app = FastAPI(
    ...
    description="Микросервис уведомлений (Email, SMS, Webhook)",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
...
async def readiness_check() -> JSONResponse:
    ...
    # TODO: Добавить проверку доступности SMTP
