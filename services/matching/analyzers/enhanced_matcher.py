"""
Расширенный сопоставитель навыков с учетом контекста и оценкой уверенности.

# Русский комментарий:
Этот модуль предоставляет интеллектуальное сопоставление навыков, которое выходит
за рамки простого сравнения строк и включает:
- Сопоставление с учетом контекста (например, React.js ≈ React в контексте web_framework)
- Оценку уверенности в качестве совпадения
- Поддержку отраслевых таксономий
- Обработку синонимов для пользовательских организаций
- Обнаружение частичных совпадений с нечетким сопоставлением
"""
import json
import logging
