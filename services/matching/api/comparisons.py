        ...
        for resume_id in resume_ids:
            try:
                ...
                try:
                    ...
                    keywords_result = extract_resume_keywords(resume_text, language=language, top_n=50)
...
async def create_comparison(
    ...
    Examples:
        ...
        ... }
        >>> response = requests.post("http://localhost:8001/api/comparisons/", json=data)
        >>> response.json()
        {
            "id": "comp-123",
            "vacancy_id": "vacancy-123",
...
async def list_comparisons(
    ...
    Examples:
        ...
        >>> response = requests.get("http://localhost:8001/api/comparisons/?vacancy_id=vac-123")
        >>> # Sort by match percentage descending
        >>> response = requests.get("http://localhost:8001/api/comparisons/?sort_by=match_percentage&order=desc")
        >>> # Filter by match percentage range
        >>> response = requests.get("http://localhost:8001/api/comparisons/?min_match_percentage=50&max_match_percentage=90")
        >>> response.json()
    """
    try:
...
async def get_comparison(
    ...
    Examples:
        ...
        ...     "http://localhost:8001/api/comparisons/123e4567-e89b-12d3-a456-426614174000"
        ... )
        >>> response.json()
    """
    try:
...
async def update_comparison(
    ...
    Examples:
        ...
        >>> response = requests.put("http://localhost:8001/api/comparisons/123", json=data)
...
async def delete_comparison(
    ...
    Examples:
        >>> import requests
        >>> response = requests.delete("http://localhost:8001/api/comparisons/123")
        >>> response.json()
...
async def compare_multiple_endpoint(request: CompareMultipleRequest) -> JSONResponse:
    ...
    Examples:
        ...
        ...     "http://localhost:8001/api/comparisons/compare-multiple",
        ...     json=data
        ... )
        >>> response.json()
        {
            ...
            if "error" in result:
                ...
                for skill_match in result.get("required_skills_match", []):
                    matched_skills.append({
                        ...
                        "matched": skill_match.get("status") == "matched",
                        "highlight": "green" if skill_match.get("status") == "matched" else "red",
                    })

                # Add additional matched skills
                for skill_match in result.get("additional_skills_match", []):
                    if skill_match.get("status") == "matched":
                        matched_skills.append({
                            "skill": skill_match.get("skill", ""),
                            "matched": True,
                            "highlight": "green",
