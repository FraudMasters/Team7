"""
Конфигурация приложения Celery для Resume Processing Service.

# Русский комментарий:
Этот модуль настраивает приложение Celery для асинхронной обработки
резюме, включая анализ текста, извлечение навыков и грамматическую проверку.
"""
import logging
import os
...
celery_app = Celery(
    ...
    include=[
        "tasks.analysis_task",  # Задачи анализа резюме / Resume analysis tasks
    ],
)
...
celery_app.conf.update(
    ...
    task_queues={
        "resume_processing": {
            "exchange": "resume_processing",
            "routing_key": "resume_processing",
        },
    },
    # Настройки маршрутизации / Routing settings
    task_default_exchange="resume_processing",
    task_default_routing_key="resume_processing",
...
# Может быть расширено для регулярного обслуживания данных
# Can be extended for regular data maintenance
celery_app.conf.beat_schedule = {
    # Пример периодической задачи (закомментирован)
    # Example periodic task (commented out)
    # "cleanup-old-results": {
    #     "task": "tasks.analysis_task.cleanup_old_analysis_results",
    #     "schedule": crontab(hour=2, minute=0),  # Ежедневно в 2:00 AM / Daily at 2:00 AM
    # },
}
...
def on_worker_ready(**kwargs):
    ...
    logger.info("Celery worker ready for Resume Processing Service")


@worker_shutdown.connect
def on_worker_shutdown(**kwargs):
    """Обработчик сигнала остановки рабочего / Worker shutdown signal handler."""
    logger.info("Celery worker shutting down for Resume Processing Service")
...
def get_celery_app() -> Celery:
    ...
    Example:
        ...
        >>> app = get_celery_app()
        >>> app.send_task('tasks.analysis_task.analyze_resume_async', args=['resume_id'])
    """
    return celery_app
