/**
 * Transparency Real-Time Updates Hook
 *
 * This module provides a React hook for managing WebSocket connections
 * specifically for real-time transparency dashboard updates.
 *
 * @example
 * ```tsx
 * import { useTransparencyRealTime } from '@/hooks/useTransparencyRealTime';
 *
 * function TransparencyDashboard() {
 *   const { isConnected, lastUpdate, refreshKey } = useTransparencyRealTime({
 *     userId: 'user-123',
 ...
 *       <ConfidenceScoreCard refreshKey={refreshKey} />
 *     </div>
 *   );
 * }
 * ```
...
/**
 * Transparency real-time hook configuration options
 */
interface UseTransparencyRealTimeOptions {
  /**
  ...
  /**
   * Callback when a transparency update is received
   * @param updateType - Type of transparency update (confidence_scores, model_accuracy, etc.)
   * @param data - The updated transparency data
   */
  onUpdate?: (updateType: TransparencyUpdateType, data: Record<string, unknown>) => void;
...
/**
 * Transparency real-time hook return value
 */
interface UseTransparencyRealTimeReturn {
  /**
  ...
  /**
   * Last received transparency update type
   */
  lastUpdate: TransparencyUpdateType | null;
...
const DEFAULT_CONFIG: Required<
  Omit<
    UseTransparencyRealTimeOptions,
...
/**
 * Transparency real-time hook for dashboard updates
 *
 * Connects to the transparency WebSocket endpoint and provides real-time
 ...
 */
export const useTransparencyRealTime = (
  options: UseTransparencyRealTimeOptions = {}
): UseTransparencyRealTimeReturn => {
  const {
  ...
  // Update tracking
  const [lastUpdate, setLastUpdate] = useState<TransparencyUpdateType | null>(null);
  ...
  const handleMessage = useCallback(
    (event: MessageEvent) => {
      try {
        ...
        switch (message.type) {
          case 'transparency_update': {
            const updateType = message.update_type as TransparencyUpdateType;
          ...
          case 'notification': {
            // Check if this is a transparency-related notification
            const notificationData = message.notification || message.data;
            if (
              notificationData?.notification_type === 'transparency_update' ||
              notificationData?.notification_type === 'confidence_score_update' ||
              notificationData?.notification_type === 'ranking_created'
            ) {
              // Trigger refresh for transparency-related notifications
              triggerRefresh();
            }
            break;
          }
...
export default useTransparencyRealTime;
