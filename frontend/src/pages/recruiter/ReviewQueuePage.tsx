...
import {
  useRecruiterReviewQueue,
  ReviewQueueFilters,
  ReviewQueueCandidateItem,
  ReviewQueueMetrics,
...
function MetricsDisplay({ metrics }: MetricsDisplayProps) {
  ...
  const metricItems = [
    {
      label: t('reviewQueue.metrics.totalPending', 'Pending'),
      value: metrics.total_pending,
      icon: <AssignmentIcon sx={{ fontSize: 20 }} />,
      color: theme.palette.primary.main,
    },
    {
      label: t('reviewQueue.metrics.urgent', 'Urgent'),
      value: metrics.urgent_count,
      icon: <WarningIcon sx={{ fontSize: 20 }} />,
      color: theme.palette.error.main,
    },
    {
      label: t('reviewQueue.metrics.avgWait', 'Avg Wait'),
      value: `${metrics.avg_wait_days.toFixed(1)}d`,
      icon: <ScheduleIcon sx={{ fontSize: 20 }} />,
      color: theme.palette.warning.main,
    },
    {
      label: t('reviewQueue.metrics.today', 'Reviewed Today'),
      value: metrics.reviewed_today,
      icon: <TodayIcon sx={{ fontSize: 20 }} />,
      color: theme.palette.success.main,
    },
    {
      label: t('reviewQueue.metrics.throughput', 'Weekly Throughput'),
      value: metrics.throughput_week,
...
interface CandidateCardProps {
  candidate: ReviewQueueCandidateItem;
        ...
        {candidate.assigned_recruiter_name && (
          <Box sx={{ mb: 2 }}>
            <Typography variant="caption" color="text.secondary" sx={{ mb: 0.5, display: 'block' }}>
              {t('reviewQueue.assignedTo', 'Assigned to')}
            </Typography>
            <Chip
              icon={<PersonIcon />}
              label={candidate.assigned_recruiter_name}
              size="small"
              variant="outlined"
              sx={{ height: 24 }}
            />
...
/**
 * Recruiter Review Queue Page
 *
 * Displays candidates awaiting recruiter review with filtering, search, and metrics.
...
export function ReviewQueuePage() {
  ...
  // Fetch review queue
  const { data: reviewQueueData, isLoading, isError, refetch } = useRecruiterReviewQueue(filters);

  // Extract data with defaults
  const candidates = reviewQueueData?.candidates || [];
  const totalCandidates = reviewQueueData?.total_candidates || 0;
  const metrics = reviewQueueData?.metrics;
    ...
    <Container maxWidth="xl" sx={{ py: 2 }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <Box>
            ...
            <Typography variant="body1" color="text.secondary">
              {t('reviewQueue.subtitleRecruiter', 'Prioritized candidates awaiting your review')}
            </Typography>
          </Box>
        ...
        <Paper sx={{ p: 6, textAlign: 'center' }}>
          ...
          <Typography color="text.secondary" sx={{ mb: 3 }}>
            {hasActiveFilters
              ...
              : t('reviewQueue.emptyMessageRecruiter', 'All caught up! New candidates will appear here when they need your review.')}
          </Typography>
          {hasActiveFilters && (
      ...
      <Box sx={{ mt: 4, p: 2, bgcolor: alpha(theme.palette.info.main, 0.1), borderRadius: 2 }}>
        <Typography variant="body2" color="text.secondary">
          ...
          {t(
            'reviewQueue.tip.contentRecruiter',
            'Candidates are sorted by priority. Click "View Details" to review the candidate profile, add feedback, and update their status.'
          )}
        </Typography>
      </Box>
    </Container>
